...
- **ko-1.1.0** — 국립국어원과 법제처의 공공언어 기준에서 문장 규칙을 보강했습니다.
  - **번역투(K2, K3):** `회의를 갖다`, `~ 중에 있다`, `~을 필요로 하다`, `~에 위치한`, `~로부터`, 필요 없는 `-시키다`와 `-고 있다`를 검토합니다. 사물이나 추상어가 사람처럼 행동하는 문장(`결과는 ~를 말해 준다`, `~를 가능하게 한다`)도 한국어식으로 다시 씁니다. 진짜 진행형과 시제, 주장의 세기는 그대로 둡니다.
  - **명사 구문(K4):** `절감이 가능하다`, `~의 경우`, `방한 시` 같은 한문 투를 서술어로 풀고, 잇따르는 `의`를 줄입니다. `약 30여 명` 같은 뚜렷한 겹말만 줄이고, `피해를 입다`나 `박수를 치다`처럼 국립국어원이 틀리지 않다고 본 표현은 그대로 둡니다.
  - **연결과 어순(K6, K8):** 나열한 말의 문법 짝을 맞추고, 접속어가 실제 논리와 맞는지 봅니다. 꾸미는 말은 꾸밈받는 말 앞에 두되, 원문이 애매하면 고르지 않고 표시합니다.
  - **외국어(K5):** `니즈`, `솔루션`처럼 장식으로 쌓인 업무 외국어만 검토합니다. 글쓴이나 분야가 평소 쓰는 말, 자리 잡은 외래어, 고유 이름은 바꾸지 않습니다.
  - **문장부호(K8):** `3~5`와 `3-5`, `2 m`와 `2m`처럼 규범이 둘 다 허용하는 표기는 통일하지 않습니다. 날짜 끝 마침표(`2024. 5. 1.`)와 한국어 줄표 삽입구를 보존하고, 강조에는 작은따옴표를 쓰며, 문장 중간의 영어식 쌍점은 문장으로 풉니다.
  - **서식:** 굵은 글씨 남발, 문장을 되풀이하는 굵은 이름표, 제목과 목록의 이모지를 한국어 글에서도 걷어냅니다. 전에는 영어 참조 문서에만 있던 규칙입니다.
  - **설명과 예시:** 작업 절차는 영어로 유지하고, 주체·사동·진행·시제·부정·접속어처럼 뉘앙스가 중요한 규칙은 한국어로 설명합니다. `센터를 건립 중`과 `센터는 건립 중`을 구분하고, `따라서`가 논리적 귀결을 나타내는 경우도 보존합니다. 같은 설명을 두 언어로 중복하지 않습니다.
  - **검사 스크립트:** `건립·증설·공사·검토·개발·진행 중에 있`, `필요로 하`, `에 위치한`을 검토 항목에 추가했습니다. `후보들 중에 있는` 같은 집합 표현은 진행형 경고에서 제외합니다. 이 목록은 모든 표현을 포괄하지 않으며, 경고는 오류 확정이 아닌 검토 후보입니다.
  - 품질 변화는 이번에 측정하지 않았습니다.
...
- [국립국어원 『(개정판) 한눈에 알아보는 공공언어 바로 쓰기』](https://www.korean.go.kr/front/etcData/etcDataView.do?mn_id=&etc_seq=699&pageIndex=1)와 [『쉬운 공문서 쓰기 길잡이』](https://www.korean.go.kr/front/etcData/etcDataView.do?mn_id=208&etc_seq=700) — 번역투와 문장 호응, 명사 나열 기준
- [국립국어원 『문장 부호 해설』](https://www.korean.go.kr/front/etcData/etcDataView.do?mn_id=46&etc_seq=431)과 [온라인가나다](https://www.korean.go.kr/front/onlineQna/onlineQnaList.do?mn_id=216) — 문장부호의 원칙과 허용, 겹말 판정
- [법제처 알기 쉬운 법령 정비기준](https://www.moleg.go.kr/menu.es?mid=a10108030000) — 수식 범위와 병렬 구조, 일본어 투 표현
