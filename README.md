...
일반 교정은 글 전체를 한 번 고친 뒤 원문과 최종안을 한 번 대조합니다. `check-rewrite.py`는 사용자가 검증을 요청했거나 수치·기술 표기가 많은 글에서 필요할 때만 쓰는 보조 도구입니다. 숫자와 표기가 달라졌는지 보여 줄 뿐 의미가 보존됐는지는 판단하지 못하므로, 결과는 반드시 원문과 비교합니다.
...
| 19 | **영어 문장의 둥근 따옴표 사용** | `She called it “the project.”` | 영어 문장에서는 `She called it "the project."`; 한국어의 `“ ”`, `‘ ’`, `「 」`, `『 』`는 유지 |
