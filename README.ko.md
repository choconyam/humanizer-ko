...
- **v2.11.1-ko.7** — 선택형 결정론 검증 스크립트 `check-rewrite.py`를 추가했습니다. 교정 후 원문의 숫자·인용문·기술 토큰이 남았는지, 상투 표현이 남았는지, 길이가 늘었는지를 모델 호출 없이 검사합니다. 스크립트를 실행할 수 없는 환경에서는 기존 체크리스트만으로 그대로 작동합니다. 수동 설치 명령도 참조 하위 폴더까지 복사하도록 고쳤습니다.
