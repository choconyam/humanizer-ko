...
블로그 글, 보고서, 이메일, 공지, 발표 대본처럼 AI 초안에서 시작한 글을 작성자가 쓴 글처럼 만들 때 사용합니다. 사실과 기술 정보뿐 아니라 작성자의 관점과 의도적인 문체도 보존합니다. AI 탐지기를 우회하는 도구가 아니라 문장을 다듬는 교정 도구입니다.
...
영어 문장이 충분한 문서에는 upstream의 35개 패턴 참조 문서를 불러옵니다. 한국어 문서에는 K1~K10을 적용하며, 영어 제품명·코드·인용·정착된 기술용어가 섞였다는 이유만으로 영어 상세 규칙까지 불러오지 않습니다. 분야별 용어 지침도 전문용어를 실제로 선택하거나 번역해야 할 때만 사용합니다.
...
먼저 원문의 제약과 장르를 기록하고 필요한 참조 문서만 불러옵니다. 글을 다듬은 뒤 주장이나 작성자의 목소리가 달라졌는지, AI 문체가 남았는지 확인하고 최종 문장을 작성합니다.

사실, 이름, 날짜, 인용문, 출처를 새로 만들어내지 않습니다. 세부 정보를 추가하려면 반드시 원문이나 작성자가 제공한 근거가 있어야 합니다. 시제, 가능성, 예정·완료 상태도 보존해 계획을 이미 끝난 일처럼 바꾸지 않습니다.

한국어 문서에는 [`references/korean-editing.md`](references/korean-editing.md)의 K1~K10을 한 번만 불러와 적용합니다. 영어 전용 규칙을 한국어에 그대로 옮기지 않고 문체, 어순, 주어, 조사, 연결어미, 문장 리듬을 별도로 판단합니다. K5 표현은 금칙어가 아니라 검토 신호로 사용하며, 정당한 인사·약속·안전 강조·승인 문구·개인 문체는 유지합니다. 장르·충실도·용어 지침은 조건에 해당할 때만 불러옵니다.
...
- **v2.11.1-ko.6** — 단어가 남았는지만 보는 방식을 문맥 중심 한국어 검토로 바꿨습니다. 교정 강도, 정상적인 인사, 공식 약속, 안전 강조, 개인 문체, 승인된 광고를 다루는 장르 지침과 의미 주장·기술 토큰을 나눠 확인하는 충실도 지침을 별도 문서로 추가했습니다. 분야 용어는 실제 선택·번역·교정이 필요할 때만 불러오고, 올바른 기존 용어를 불필요하게 바꾸지 않도록 강화했습니다.
- **v2.11.1-ko.5** — A/B 비교 결과를 바탕으로 스킬 구조를 고쳤습니다. 라우팅 프롬프트를 짧게 만들고, 영어 35개 상세 패턴은 영어 문장 구간이 충분할 때만, 분야별 용어 지침은 용어를 실제로 선택하거나 번역해야 할 때만 불러옵니다. 한국어 상투 표현 의무 점검, 일반 교정의 최종 문장만 출력, 시제·가능성·예정·완료 상태 보존 규칙도 추가했습니다.
