...
먼저 원문의 제약을 기록하고 글의 언어에 필요한 참조 문서만 불러옵니다. 글을 다듬은 뒤 주장이 달라졌거나 AI 문체가 남았는지 한 번 확인하고 최종 문장을 작성합니다.

사실, 이름, 날짜, 인용문, 출처를 새로 만들어내지 않습니다. 세부 정보를 추가하려면 반드시 원문이나 작성자가 제공한 근거가 있어야 합니다. 시제, 가능성, 예정·완료 상태도 보존해 계획을 이미 끝난 일처럼 바꾸지 않습니다.

한국어 문서에는 [`references/korean-editing.md`](references/korean-editing.md)의 K1~K10을 한 번만 불러와 적용합니다. 영어 전용 규칙을 한국어에 그대로 옮기지 않고 문체, 어순, 주어, 조사, 연결어미, 문장 리듬을 별도로 판단합니다. 최종 출력 직전에는 K5 상투 표현을 반드시 다시 확인합니다. [`references/domain-terminology.md`](references/domain-terminology.md)은 용어 선택이 필요한 경우에만 불러와 분야, 공식 기준, 문서 종류, 독자에 맞는 용어를 고릅니다.
